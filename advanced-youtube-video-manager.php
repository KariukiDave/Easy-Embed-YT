...
class AdvancedYouTubeVideo {
    ...
    private function render_analytics_tab() {
        global $wpdb;
        $table_name = $wpdb->prefix . 'youtube_videos';
        $videos = $wpdb->get_results("SELECT id, title, view_count FROM $table_name ORDER BY view_count DESC");
    ...
    /**
     * AJAX handler for tracking video plays
     */
    ...
    /**
     * Legacy embed function (backward compatible)
     */
...
function advanced_youtube_video_activate() {
    ...
    // Tables will be created automatically in constructor
}
...
function advanced_youtube_video_inline_scripts() {
    ...
    <div id="advanced-youtube-lightbox" class="advanced-youtube-lightbox">
        <div class="advanced-youtube-lightbox-content">
            ...
            <div id="lightbox-video-container" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden;">
            </div>
        </div>
    </div>
    ...
    jQuery(document).ready(function($) {
        ...
        // Auto-copy shortcode when new video is added (if visible)
        $('.shortcode-copy').first().each(function() {
            var $notice = $(this).closest('.notice-success');
            if ($notice.length > 0) {
                // Auto-copy the shortcode for newly added videos
                $(this).trigger('click');
            }
        });
...
?>
            return '<p><em// Prevent direct access
if (!defined('ABSPATH')) {
    exit;
}
...
?>>No active YouTube video found.</em></p>';
